...
export default function ChatWidget() {
  ...
  const initChat = async () => {
    ...
    try {
      ...
      channelRef.current = supabase
        .channel(`messages:${d.data.id}`)
        .on('postgres_changes',
          { event: 'INSERT', schema: 'public', table: 'messages', filter: `conversation_id=eq.${d.data.id}` },
          (payload) => {
            setMessages(prev => {
  ...
  return (
    <>
      <button
        onClick={() => setOpen(p => !p)}
        className="fixed bottom-6 left-6 z-[9999] w-14 h-14 rounded-full bg-brand text-white shadow-lg hover:bg-brand-dark transition-all cursor-pointer flex items-center justify-center"
      >
      ...
      {open && (
        <div className="fixed bottom-24 left-6 z-[9999] w-[360px] max-w-[calc(100vw-2rem)] h-[500px] max-h-[calc(100vh-200px)] bg-white rounded-2xl shadow-2xl border border-zinc-200 flex flex-col overflow-hidden" style={{ direction: 'rtl' }}>
          <div className="bg-brand text-white px-5 py-4 flex items-center gap-3 shrink-0">
              ...
              messages.map(msg => (
                <div key={msg.id} className={`flex ${msg.sender_role === 'admin' ? 'justify-start' : 'justify-end'}`}>
                  <div className={`max-w-[80%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${
                    ...
                    <p>{msg.content}</p>
                    <p className={`text-[10px] mt-1 ${msg.sender_role === 'admin' ? 'text-zinc-400' : 'text-white/60'}`}>
