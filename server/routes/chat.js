...
function sb() {
  const s = getSupabase()
  if (!s) throw new ApiError(503, 'Supabase not configured')
  return s
}
...
router.get('/', adminRequired, catchAsync(async (req, res) => {
  ...
  const conversations = await Promise.all((data || []).map(async (conv) => {
    const { data: msgs } = await supabase.from('messages').select('content, created_at, sender_role').eq('conversation_id', conv.id).order('created_at', { ascending: false }).limit(1)
...
router.get('/my', authRequired, catchAsync(async (req, res) => {
  ...
  let { data: conv } = await supabase.from('conversations').select('*').eq('user_id', userId).eq('status', 'active').single()
  if (!conv) {
    const { data: newConv, error } = await supabase.from('conversations').insert({ user_id: userId, user_name: req.user.name || '', user_email: req.user.email || '' }).select().single()
    if (error) return res.status(500).json({ message: error.message })
    conv = newConv
  }
  res.json({ data: conv })
...
router.post('/:id/message', authRequired, catchAsync(async (req, res) => {
  ...
  const { data, error } = await supabase.from('messages').insert({ conversation_id: req.params.id, sender_id: String(req.user._id), sender_role: senderRole, content: content.trim() }).select().single()
  if (error) return res.status(500).json({ message: error.message })
...
router.patch('/:id/resolve', adminRequired, catchAsync(async (req, res) => {
  const supabase = sb()
  const { data, error } = await supabase.from('conversations').update({ status: 'resolved', updated_at: new Date().toISOString() }).eq('id', req.params.id).select().single()
  if (error) return res.status(500).json({ message: error.message })
  res.json({ data })
}))
...
router.patch('/:id/reopen', adminRequired, catchAsync(async (req, res) => {
  const supabase = sb()
  const { data, error } = await supabase.from('conversations').update({ status: 'active', updated_at: new Date().toISOString() }).eq('id', req.params.id).select().single()
  if (error) return res.status(500).json({ message: error.message })
  res.json({ data })
}))
